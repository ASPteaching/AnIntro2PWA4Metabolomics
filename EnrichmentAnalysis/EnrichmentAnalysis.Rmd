...
subtitle: "ORA and MSEA for metabolomics"
author: "Alex Sanchez"
date: "`r Sys.Date()`"
output:
  html_document:
...
MSEA in `localEnrichment` has been written to match MetaboAnalyst function so it is also called `qea_test`.
...
`EnrichMet` implements MSEA (referred to as **MetSEA**) using the **fgsea algorithm**, which corresponds to a fast implementation of the GSEA running-sum statistic.
...
Overall, both `localEnrichment` and `EnrichMet` implement conceptually the same class of QEA/MSEA methods (GSEA-like), differing mainly in computational backend and workflow integration rather than in statistical principles.

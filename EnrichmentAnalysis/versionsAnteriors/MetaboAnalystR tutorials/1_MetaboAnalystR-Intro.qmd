...
The current document is a short introduction to the basic steps that have to be followed in order to read and preprocess a simple dataset of concentrations.
...
The first step to use MetaboAnalyst is to create a *container*, which is simply an object of a special class, `mSet`, designed to facilitate the storage and manipulation of data, usually for further  Statistical Analysis.
...
For this lab we use a preprocessed dataset that we download from the prgram's web site.

```{r getData}
download.file("https://rest.xialab.ca/api/download/metaboanalyst/human_cachexia.csv",destfile = "./human_cachexia.csv")
```

Opening the file after the data has been downloaded shows the structure data usually managed by MetaboAnalyst

```{r}
cachexiaOriginal <- read.csv("./human_cachexia.csv")
head(cachexiaOriginal)
```

We don't need to worry about this structure because this is intermanaged by the `mSet` class.

As next step we proceed to read the data into the `mSet` object previously created and we check its validty
